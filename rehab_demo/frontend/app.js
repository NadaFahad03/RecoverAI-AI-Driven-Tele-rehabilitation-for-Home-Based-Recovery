...
function renderResult(r) {
  ...
  if (!r.exercise_match) {
  $("resultCard").classList.add("is-bad");
  $("resultIcon").innerHTML = ICONS.cross;

  $("resultVerdict").textContent = "EXERCISE MISMATCH";

  $("resultConfidence").textContent =
    `${(r.exercise_confidence * 100).toFixed(0)}% match confidence`;

  $("resultExercise").textContent =
    `Selected: ${r.exercise_name} • Detected: ${r.predicted_exercise_name}`;

  requestAnimationFrame(() => {
    $("confidenceFill").style.width =
      `${(r.exercise_confidence * 100).toFixed(1)}%`;
  });
  ...
  if (r.overlay_url) {
    $("overlayVideo").src = `http://127.0.0.1:8000${r.overlay_url}`;
